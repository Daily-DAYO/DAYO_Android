...
class PostReportFragment : Fragment() {
    ...
    private fun setOnClickPostReportOtherReason() {
        binding.radiogroupPostReportReason.setOnCheckedChangeListener { _, id ->
            if (binding.radiogroupPostReportReason.checkedRadioButtonId != -1) binding.btnPostReport.isSelected =
                true
            when (id) {
                binding.radiobuttonPostReportReasonOther.id -> binding.etPostReportReasonOther.visibility =
                    View.VISIBLE
    ...
    private fun setPostReportButtonClickListener() {
        binding.btnPostReport.setOnClickListener {
            if (binding.btnPostReport.isSelected) {
                Toast.makeText(
                    requireContext(),
                    getString(R.string.post_report_alert_message),
                    Toast.LENGTH_SHORT
                ).show()
                findNavController().navigateUp()
            }
