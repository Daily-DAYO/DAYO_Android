...
class LoginRepository @Inject constructor(private val loginApiHelper: LoginApiHelper) {
    suspend fun requestLoginKakao(request: LoginRequestKakao): Response<LoginResponse> =
        loginApiHelper.requestLoginKakao(request)

    suspend fun requestLoginEmail(request: LoginRequestEmail): Response<LoginResponse> =
        loginApiHelper.requestLoginEmail(request)
    ...
    suspend fun requestRefreshToken(): Response<ResponseRefreshToken> =
        loginApiHelper.requestRefreshToken()

    suspend fun requestDeviceToken(request: RequestDeviceToken): Response<Void> =
        loginApiHelper.requestDeviceToken(request)
    ...
    suspend fun requestSignupEmail(
        ...
        var requestBodyFile: RequestBody =
    ...
    suspend fun requestCheckEmailDuplicate(email: String): Response<Void> =
        loginApiHelper.requestCheckEmailDuplicate(email)

    suspend fun requestCertificateEmail(email: String): Response<SignupEmailAuthCodeResponse> =
        loginApiHelper.requestCertificateEmail(email)

    suspend fun requestChangeLostPassword(email: String, password: String): Response<Void> =
        loginApiHelper.requestChangeLostPassword(RequestChangePassword(email, password))
}
